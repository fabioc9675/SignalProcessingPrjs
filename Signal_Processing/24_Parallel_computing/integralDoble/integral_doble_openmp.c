...
double compute_distance(Point p1, Point p2) {
    ...
    double dy = p1.y - p2.y;
    double dz = p1.z - p2.z;
    return sqrt(dx*dx + dy*dy + dz*dz + 1e-15); // +1e-15 para evitar división por cero
}

int main() {
    // Leer data1.txt
    FILE *file1 = fopen("data1.txt", "r");
    Point *data1 = malloc(500000 * sizeof(Point)); // Ajusta el tamaño
    int N = 0;
    ...
    Point *data2 = malloc(500000 * sizeof(Point));
    int M = 0;
