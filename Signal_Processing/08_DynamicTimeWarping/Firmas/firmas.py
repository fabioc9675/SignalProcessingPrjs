...
def on_release(event):
    ...
    plt.close(fig)  # Cierra la ventana automáticamente
    process_and_save_signature(raw_signature)
...
def save_signature_image(filename='firma_t.png'):
    ax.set_title("Firma capturada")
    ax.axis('off')
    fig.savefig(filename, bbox_inches='tight')
    print(f"Imagen de la firma guardada como '{filename}'")

def process_and_save_signature(signature_data, filename='firma_t.csv'):
